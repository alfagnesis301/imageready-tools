      ...
      <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
        <div>
          ...
          <p className="mt-4 text-sm leading-7 text-slate-600 dark:text-slate-400">
            {isEs
              ? "Analiza el peso, las dimensiones, el formato, el nombre del archivo y la preparacion de publicacion de una imagen en un solo flujo. Es util para revisar activos antes de subirlos al CMS, al blog o a una vista previa social."
          ...
          <p className="mt-4 text-sm leading-7 text-slate-600 dark:text-slate-400">
            {isEs
              ? "La herramienta vive dentro del producto principal, usa el flujo real de PublishPixel y ya esta pensada para capturar busquedas sobre image SEO checker, image meta checker, website image optimiser y social media image sizes."
        ...
        <aside className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <ul className="mt-4 grid gap-3 text-sm leading-7 text-slate-700 dark:text-slate-300">
            <li>{isEs ? "Peso del archivo y oportunidad de compresion" : "File weight and compression opportunity"}</li>
            <li>{isEs ? "Dimensiones y proporcion para web y social" : "Dimensions and aspect ratio for web and social"}</li>
            ...
            <li>{isEs ? "Senales practicas antes de publicar" : "Practical pre-publish signals"}</li>
          </ul>
        </aside>
      </div>
    </section>
